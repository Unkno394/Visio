...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'meetings'>('meetings');
  ...
  const generateRoomId = () => {
    const useCrypto = typeof crypto !== "undefined" && crypto;
    const random =
      useCrypto && typeof crypto.randomUUID === "function"
        ? crypto.randomUUID()
        : (() => {
            if (useCrypto && typeof crypto.getRandomValues === "function") {
  ...
  const handleCreateRoom = () => {
    ...
    router.push(`/create?roomId=${roomId}&owner=1`);
  };

  const handleJoinRoom = (roomId: string) => {
    router.push(`/create?roomId=${roomId}`);
  };
